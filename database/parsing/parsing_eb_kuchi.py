...
with open (os.path.abspath('../static_data/json_files/eb_kuchi_data.json')) as f:
    json = json.load(f)
    f.close()
...
for i in range(len(json['products'])):
    ...
    if (json['products'][i]['productName'] != 'Sewing Machine Project Donation Card'):
        ...
        current_product['price'] = float(json['products'][i]['priceInGBP'][2:])

        # Use helper function to convert currency symbol to 3 char string representation
        current_product['currency'] = curr_symbol_to_code(json['products'][i]['priceInGBP'][1:2])
        
        current_product['productUrl'] = json['products'][i]['url']
        
        current_product['gender'] = 'F'
        
        current_product['productDetails'] = json['products'][i]['productDetails'][0]['data'].replace('\n', " ")
        
        current_product['mainImageUrl'] = json['products'][i]['productImage_url']

        # need to modify scraping structure to include size measurements, using filler data for now
        sizeList = get_sizes(json['products'][i]['productDetails'][0]['data'].replace('\n', " "))
...
csv_file = '../../backend/project/product_data/parsed_eb_kuchi.csv'
